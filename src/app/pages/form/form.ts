...
export class Form implements OnInit {
  ...
  validateCurrentStep(): boolean {
    ...
    if (this.currentStep === 1) {
      if (!this.PaintType) {
        this.errors['PaintType'] = 'Please select a service.';
        valid = false;
      }
      ...
      if (!this.BriefRequirement.trim()) {
        this.errors['BriefRequirement'] = 'Please provide a brief description of your painting needs.';
        valid = false;
      }
  ...
  async submit() {
    ...
    if (this.validateCurrentStep()) {
      ...
      const payload = {
        PaintType: parseInt(this.PaintType),
      ...
      this.http.post('https://get-painting.com/server/forward-lead', payload).subscribe({
        next: (response) => {
